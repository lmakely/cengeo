# This version of the script is for submissions that came in through SWIM.
import os, arcpy, zipfile, shutil, datetime, sqlite3, winsound
arcpy.env.outputMFlag = "Disabled"
arcpy.env.outputZFlag = "Disabled"
swimdir = r'\\batch4.ditd.census.gov\mtdata003_geo_SWIM\BAS\BAS2016'#
#swimdir = r'\\batch4.geo.census.gov\mtdata003_geoarea\BAS\Digital_BAS_2013\Cycle1DigitalFinalQCArchive'#change before production
#basdir = r'\\Npc-apps\incoming\BAS\Digital_BAS_2016\processoing' #NPC Version
basdir = r'\\batch4.ditd.census.gov\mtdata003_geoarea\BAS\Digital_BAS_2016\processing'#HQ version
#basdir = r'\\batch4.geo.census.gov\mtdata003_geoarea\BAS\Digital_BAS_2015\test'#change before production
#blisrds = r'\\Npc-apps\incoming\BAS\Digital_BAS_2016\Tools\BLISRDS_STAGE' #NPC     version will not work due to BLISRDS constraints
blisrds = r'\\batch4.ditd.census.gov\mtdata003_geoarea\BAS\BLISRDS_STAGE'#HQ and NPC version
bnchdir = r'\\batch4.ditd.census.gov\mtdata003_geo_shpgen\mtps_mtdb\bas16_2015'#
#bnchdir = r'\\batch4.geo.census.gov\mtdata003_geo_shpgen\mtps_mtdb\acs14_2014'#change before production
bnchBase = 'bas16_2015_'#
#bnchBase = 'acs14_2014_'#change before production
##bas_y_ycounty_merge = r'\\Npc-apps\incoming\BAS\Digital_BAS_2016\Tools\Templates\bas16countymerge.shp' #NPC version non VDI version
BASyycountyMerge = r'\\batch4.ditd.census.gov\mtdata003_geoarea\BAS\ABEUS\bas16countymerge.shp'#HQ version
logdb = basdir + '\\basSetup.db'
mxdtemplate = r'\\batch4.ditd.census.gov\mtdata003_geoarea\BAS\Digital_BAS_2016\Templates\preprocess_template3.mxd'
#mxdtemplate = r'\\Npc-apps\incoming\BAS\Digital_BAS_2016\Tools\Templates\preprocess_template.mxd' #NPC non VDI version
#mxdtemplate = r'\\batch4.geo.census.gov\mtdata003_geoarea\BAS\ABEUS\preprocess_template.mxd' #HQ version
...
def bnchCountyMerge():#use to create merged file of counties 'BAS16countyMerge'
...
def lisrds(countiesList): # this function is used to create a txt file to invoke blisrds
    for cou in countiesList:
        f= open(blisrds+'\\'+cou+'.txt','w')
        f.close()

def whichZip(srcdir, basid): # This is used if more than one zip file exists for a given basid
    results=[]
...
#figure out MTPS vs digital vs GUPS
...
#the FD is already projected, so the files will be forced into projection on import.
    ...
    outputFC = 'bas16_' + basid + '_changes_' + chngtype #UPDATE THE YEAR on this! every year!
...
def runit(basid, supervised=True):
    ...
    try:
        ...
        if subType == 'GUPS': # place holder in case we need to do special work for GUPS related return files.
            pass
